...
public:
    ...
    testGWB(std::initializer_list<uint256> fs)
    {
        ...
        if (wsc->version() == 2)
        {
            ...
            expect(jv.isMember(jss::callrpc) && jv[jss::callrpc] == "2.0");
...
BEAST_DEFINE_TESTSUITE(GatewayBalances,app,call);
