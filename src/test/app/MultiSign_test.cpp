...
#include <call/protocol/JsonFields.h>     // jss:: definitions
...
public:
    void test_noReserve()
    {
...

        {
            ...
            env(pay(env.master, alice, CALL(151)));
            env.close();
            env(smallSigners);
            env.close();
        ...
        }
        {
            ...
            env(pay(env.master, alice, CALL(350)));
            env.close();
            env(bigSigners);
            env.close();
...
//      env(signers(alice, 1, { { bogie, 0x10000} }), ter (temBAD_WEIGHT));
    ...
    void test_masterSigners()
    {
        ...
        env.fund(CALL(1000), alice, becky, cheri);
        env.close();
    ...
    void test_heterogeneousSigners()
    {
        ...
        env.fund(CALL(1000), alice, becky, cheri, daria);
        env.close();
    ...
    void test_txTypes()
    {
        ...
        env.fund(CALL(1000), alice, becky, zelda, gw);
        env.close();
        ...
        auto const baseFee = env.current()->fees().base;
        std::uint32_t aliceSeq = env.seq (alice);
        env(pay(alice, env.master, CALL(1)),
    ...
    void test_noMultiSigners()
    {
        ...
        Account const alice {"alice", KeyType::ed25519};
        Account const becky {"becky", KeyType::secp256k1};
        env.fund(CALL(1000), alice, becky);
        env.close();
...
BEAST_DEFINE_TESTSUITE(MultiSign, app, call);
