...
auto IPE(Issue const& iss)
{
    return STPathElement (
        ...
        callAccount (), iss.currency, iss.account);
};
...
public:
                    ...
                    boost::optional<Currency> const& saSrcCurrency = boost::none)
    {
        ...
        params[jss::command] = "call_path_find";
    ...
    path_find_consume_all()
    {
...

        {
            ...
            env(offer("carol", CALL(100), USD(100)));
            ...
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["AUD"](-1),
                    boost::optional<STAmount>(CALL(100000000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["USD"](-1),
                    boost::optional<STAmount>(CALL(100000000)));
            BEAST_EXPECT(sa == CALL(100));
    ...
    alternative_path_consume_both()
    {
        ...
        env.fund(CALL(10000), "alice", "bob", gw, gw2);
        env.trust(USD(600), "alice");
    ...
    alternative_paths_limit_returned_paths_to_best_quality()
    {
        ...
        auto const gw2 = Account("gateway2");
        auto const gw2_USD = gw2["USD"];
        env.fund(CALL(10000), "alice", "bob", "carol", "dan", gw, gw2);
        env(rate("carol", 1.1));
    ...
    void
    issues_path_negative_call_client_issue_23_smaller()
    {
        testcase("path negative: call-client issue #23: smaller");
        using namespace jtx;
        Env env(*this);
        env.fund(CALL(10000), "alice", "bob", "carol", "dan");
    ...
    void
    issues_path_negative_call_client_issue_23_larger()
    {
        testcase("path negative: call-client issue #23: larger");
        using namespace jtx;
        Env env(*this);
        env.fund(CALL(10000), "alice", "bob", "carol", "dan", "edward");
    ...
    // carol holds gateway AUD, sells gateway AUD for CALL
    // bob will hold gateway AUD
    // alice pays bob gateway AUD using CALL
    void
    via_offers_via_gateway()
    {
        ...
        env(offer("carol", CALL(50), AUD(50)));
        env(pay("alice", "bob", AUD(10)), sendmax(CALL(100)), paths(CALL));
    ...
    void path_find_01()
    {
        testcase("Path Find: CALL -> CALL and CALL -> IOU");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(CALL(100000), A1);
        env.fund(CALL(10000), A2);
        env.fund(CALL(1000), A3, G1, G2, G3, M1);
        env.close();
        ...
        env(offer(M1, CALL(10000), G3["ABC"](1000)));

        STPathSet st;
        STAmount sa, da;

        {
            auto const& send_amt = CALL(10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, callCurrency());
...

        {
            ...
            std::tie(st, sa, da) = find_paths(env, A2, G3, send_amt,
                boost::none, callCurrency());
            BEAST_EXPECT(equal(da, send_amt));
            BEAST_EXPECT(equal(sa, CALL(100)));
    ...
    void path_find_02()
    {
        testcase("Path Find: non-CALL -> CALL");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(CALL(1000), A1, A2, G3);
        env.fund(CALL(11000), M1);
        env.close();
        ...
        env(offer(M1, G3["ABC"](1000), CALL(10000)));
        ...
        BEAST_EXPECT(same(st, stpath(G3, IPE(callIssue()))));
    }
    ...
    void path_find_03()
    {
        ...
        env.fund(CALL(4999.999898), SRC);
        env.fund(CALL(10846.168060), GATEWAY_DST);
        env.fund(CALL(4291.430036), MONEY_MAKER_1);
        env.fund(CALL(106839.375770), MONEY_MAKER_2);
        env.fund(CALL(1240.997150), A1);
        env.fund(CALL(14115.046893), A2);
        env.fund(CALL(512087.883181), A3);
        env.close();
        ...
        env(offer(MONEY_MAKER_2, CALL(1), GATEWAY_DST["CNY"](1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](1), CALL(1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](318000), CALL(53000)));
        env(offer(MONEY_MAKER_2, CALL(209), MONEY_MAKER_2["CNY"](4.18)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](990000), CALL(10000)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](9990000), CALL(10000)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](8870000), CALL(10000)));
        env(offer(MONEY_MAKER_2, CALL(232), MONEY_MAKER_2["CNY"](5.568)));
        env(offer(A2, CALL(2000), MONEY_MAKER_1["CNY"](66.8)));
        env(offer(A2, CALL(1200), GATEWAY_DST["CNY"](42)));
        env(offer(A2, MONEY_MAKER_1["CNY"](43.2), CALL(900)));
        env(offer(A3, MONEY_MAKER_1["CNY"](2240), CALL(50000)));
        ...
        std::tie(st, sa, da) = find_paths(env, SRC, GATEWAY_DST, send_amt,
            boost::none, callCurrency());
        BEAST_EXPECT(equal(da, send_amt));
        BEAST_EXPECT(equal(sa, CALL(288.571429)));
    ...
    void path_find_04()
    {
        ...
        env.fund(CALL(1000), G1BS, G2SW, A1, A2);
        env.fund(CALL(11000), M1);
        env.close();
        ...
        // can charge a fee when transactions call across. Liquidity
    ...
    void path_find_05()
    {
        testcase("Path Find: non-CALL -> non-CALL, same currency");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(CALL(1000), A1, A2, A3, G1, G2, G3, G4);
        env.fund(CALL(10000), A4);
        env.fund(CALL(11000), M1, M2);
        env.close();
        ...
        env(offer(M2, CALL(10000), G2["HKD"](1000)));
        env(offer(M2, G1["HKD"](1000), CALL(10000)));
...

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(M1),
                stpath(M2),
                stpath(IPE(callIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(callIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            //I4) CALL bridge" --
            //  Source -> AC -> OB to CALL -> OB from CALL -> AC -> Destination
            auto const& send_amt = A2["HKD"](10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, G1["HKD"].currency);
            BEAST_EXPECT(equal(da, send_amt));
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(callIssue()), IPE(G2["HKD"]), G2)
            ));
        }
    }
    ...
    void path_find_06()
    {
        ...
        env.fund(CALL(11000), M1);
        env.fund(CALL(1000), A1, A2, A3, G1, G2);
        env.close();
...
BEAST_DEFINE_TESTSUITE(Path,app,call);
