...
class TxQ_test : public beast::unit_test::suite
{
        ...
        std::uint32_t units, std::uint32_t reserve, std::uint32_t increment)
    {
        ...
        checkMetrics(env, 0,
            2 * (call::detail::supportedAmendments().size() + 1),
...
public:
    void testQueue()
    {
        ...
        env.fund(CALL(50000), nocall(alice, bob, charlie, daria));
        ...
        env.fund(CALL(50000), nocall(elmo, fred, gwen, hank));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(CALL(50000), nocall(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(CALL(1000), nocall(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(CALL(1000), nocall(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(CALL(1000), nocall(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(CALL(1000), nocall(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(CALL(1000), nocall(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(CALL(1000), nocall(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, CALL(-1000)),
            ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(CALL(100000)),
            ter(terINSUF_FEE_B));
    }
    ...
    void testMultiTxnPerAccount()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(drops(2000), nocall(alice));
        env.fund(CALL(500000), nocall(bob, charlie, daria));
        ...
        aliceFee = env.le(alice)->getFieldAmount(sfBalance).call().drops()
            - (59);
        env(noop(alice), seq(aliceSeq),
        ...
        // Alice is broke
        env.require(balance(alice, CALL(0)));
    ...
    void testDisabled()
    {
        ...
        env.fund(CALL(50000), nocall(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(drops(5000), nocall(alice));
        env.fund(CALL(50000), nocall(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more CALL, and it'll process.
        */
    ...
    void testBlockers()
    {
        ...
        env.fund(CALL(50000), nocall(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(CALL(50000), nocall(alice, charlie), gw);
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        //////////////////////////////////////////
        // Offer with high CALL out blocks later txs
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, CALL(50000)),
            owners(alice, 0));

        // If this offer crosses, all of alice's
        // CALL will be taken (except the reserve).
        env(offer(alice, BUX(5000), CALL(50000)),
            queued);
        ...
        // has plenty of CALL, because the offer didn't
        // cross (of course).
        env.require(balance(alice, aliceBal - drops(20)),
            owners(alice, 1));

        //////////////////////////////////////////
        // Offer with low CALL out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 8, 5, 4, 256);
        aliceSeq = env.seq(alice);
        ...
        // of alice's CALL will be taken.
        env(offer(alice, BUX(50), CALL(500)),
            queued);
        ...
        //////////////////////////////////////////
        // Large CALL payment blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 10, 6, 5, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, CALL(50000)),
            queued);
        ...
        //////////////////////////////////////////
        // Small CALL payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 12, 7, 6, 256);
        ...
        env(pay(alice, charlie, CALL(500)),
            queued);
        ...
        env.require(balance(alice, aliceBal - CALL(500) - drops(20)),
            owners(alice, 2));
        ...
        // alice's CALL balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        ...
        // CALL balance, but her USD balance went to charlie.
        env.require(balance(alice, aliceBal - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Large CALL to IOU payment blocks later txs.

        env(offer(gw, CALL(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire CALL balance to charlie in the
        // form of USD.
        BEAST_EXPECT(CALL(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)),
            sendmax(CALL(60000)), queued);
        ...
        // to charlie using only a portion of her CALL balance
        env.require(balance(alice, aliceBal - CALL(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small CALL to IOU payment allows later txs.
        ...
        // a portion of her CALL balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > CALL(6001));
        env(pay(alice, charlie, USD(500)),
            sendmax(CALL(6000)), queued);
        ...
        // So once we close the ledger, alice sent a payment
        // to charlie using only a portion of her CALL balance
        env.require(balance(alice, aliceBal - CALL(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(CALL(500000), nocall(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testSignAndSubmitSequence()
    {
        ...
        auto const alice = Account("alice");
        auto const bob = Account("bob");
        env.fund(CALL(100000), alice, bob);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(CALL(50000), nocall(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(CALL(50000), nocall(e,f,g,h,i));
    ...
    void testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(CALL(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(CALL(1)), seq(aliceSeq++), ter(terQUEUED));
...
BEAST_DEFINE_TESTSUITE(TxQ,app,call);
