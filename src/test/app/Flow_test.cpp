...
callMinusFee (jtx::Env const& env, std::int64_t callAmount)
{
    ...
    return drops (
        dropsPerCALL<std::int64_t>::value * callAmount - feeDrops);
};
...
struct Flow_test : public beast::unit_test::suite
{
    ...
    void testDirectStep (std::initializer_list<uint256> fs)
    {
        ...
        }
        {
            ...
            env (pay (alice, bob, CALL (100)));
            env.require (balance (bob, CALL (10000 + 100)));
            env.require (balance (alice, callMinusFee (env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env (*this, with_features(fs));
        ...
        }
        {
            ...
            env.fund (CALL (10000), alice, bob, carol, dan);
        ...
        }
        {
            ...
            env.fund (CALL (10000), alice, bob, carol, dan, erin);
    ...
    void testBookStep (std::initializer_list<uint256> fs)
    {
        ...
        {
            // simple IOU/CALL CALL/IOU offer
            Env env (*this, with_features(fs));
            ...
            env (offer (bob, BTC (50), CALL (50)));
            env (offer (bob, CALL (50), USD (50)));

            env (pay (alice, carol, USD (50)), path (~CALL, ~USD),
                sendmax (BTC (50)));
            ...
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, CALL (50), USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BTC (50), CALL (50)));
        }
        {
            // simple CALL -> USD through offer and sendmax
            Env env (*this, with_features(fs));
            ...
            env.require (balance (bob, callMinusFee (env, 10000 + 50)));
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, CALL (50), USD (50)));
        }
        {
            // simple USD -> CALL through offer and sendmax
            Env env (*this, with_features(fs));
            ...
            env (offer (bob, USD (50), CALL (50)));

            env (pay (alice, carol, CALL (50)), path (~CALL),
                 sendmax (USD (50)));

            env.require (balance (alice, USD (0)));
            env.require (balance (bob, callMinusFee (env, 10000 - 50)));
            env.require (balance (bob, USD (50)));
            env.require (balance (carol, CALL (10000 + 50)));
            BEAST_EXPECT(!isOffer (env, bob, USD (50), CALL (50)));
        }
        {
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (1)), path (~CALL, ~EUR),
    ...
    void testTransferRate (std::initializer_list<uint256> fs)
    {
        ...
        }
        {
            ...
            env (pay (alice, carol, USD (50)), path (~USD), sendmax (CALL (50)));
            env.require (
                balance (alice, callMinusFee (env, 10000 - 50)),
...

        {
            ...
            env (pay (alice, carol, EUR (40)), path (~USD, ~EUR), sendmax (CALL (40)));
            env.require (
                balance (alice,  callMinusFee (env, 10000 - 40)),
                balance (bob, USD (40)),
                balance (bob, EUR (0)),
        ...
        }
        {
            ...
            env.fund (CALL (10000), alice, bob, carol, dan, gw);
            env(rate(gw, 1.25));
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, CALL (100), USD (100)));
            env (pay (alice, bob, USD (100)),
                 sendmax (CALL (100)));
            env.require (
                balance (alice, callMinusFee(env, 10000-100)),
                balance (bob, USD (100)));
        }
        if (!hasFeature(featureOwnerPaysFee, fs))
        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, CALL (125), USD (125)));
            env (pay (alice, bob, USD (100)),
                 sendmax (CALL (200)));
            env.require (
                balance (alice, callMinusFee(env, 10000-125)),
                balance (bob, USD (100)));
        }
    }
    ...
    testFalseDry(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (CALL(10000), alice, carol, gw);
        ...
        // Bob has _just_ slightly less than 50 call available
        // If his owner count changes, he will have more liquidity.
        // This is one error case to test (when Flow is used).
        // Computing the incomming call to the CALL/USD offer will require two
        // recursive calls to the EUR/CALL offer. The second call will return
        // tecPATH_DRY, but the entire path should not be marked as dry. This
        // is the second error case to test (when flowV1 is used).
        env (offer (bob, EUR (50), CALL (50)));
        env (offer (bob, CALL (50), USD (50)));

        env (pay (alice, carol, USD (1000000)), path (~CALL, ~USD),
            sendmax (EUR (500)),
    ...
    testLimitQuality ()
    {
        ...
        for (auto const& d : {-100 * timeDelta, +100 * timeDelta})
        {
            ...
            env (offer (bob, CALL (50), USD (50)));
            env (offer (bob, CALL (100), USD (50)));

            auto expectedResult =
                closeTime < fix1141Time () ? tecPATH_DRY : tesSUCCESS;
            env (pay (alice, carol, USD (100)), path (~USD), sendmax (CALL (100)),
    ...
    static CALLAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    void testSelfFundedCALLEndpoint (bool consumeOffer, std::initializer_list<uint256> fs)
    {
        // Test that the deferred credit table is not bypassed for
        // CALLEndpointSteps. If the account in the first step is sending CALL and
        // that account also owns an offer that receives CALL, it should not be
        // possible for that step to use the CALL received in the offer as part
        // of the payment.
        testcase("Self funded CALLEndpoint");
        ...
        env(offer(alice, CALL(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend), path(~USD), sendmax(CALL(20000)),
    ...
    void testUnfundedOffer (bool withFix, std::initializer_list<uint256> fs)
    {
        ...
        using namespace jtx;
        {
            ...
            env.fund(CALL(100000), alice, bob, gw);
            ...
            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, CALL(0), USD(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, CALL(0), USD(0)));
        }
        {
            ...
            env(pay(alice, bob, drops(9000000000)), path(~CALL),
                sendmax(USD(1)), txflags(tfNoCallDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, USD(0), CALL(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, USD(0), CALL(0)));
        }
    }
    ...
    testReexecuteDirectStep(std::initializer_list<uint256> fs)
    {
        ...
        env(pay(alice, bob, CALL(10000)), path(~CALL), sendmax(USD(100)),
            txflags(tfPartialPayment | tfNoCallDirect));
    }
    ...
    testRIPD1443(bool withFix)
    {
        ...
        env.fund(CALL(100000000), alice, nocall(bob), carol, gw);
        env.trust(gw["USD"](10000), alice, carol);
        env(trust(bob, gw["USD"](10000), tfSetNoCall));
        env.trust(gw["USD"](10000), bob);
        env.close();

        // set no call between bob and the gateway

        env(pay(gw, alice, gw["USD"](1000)));
        env.close();

        env(offer(alice, bob["USD"](1000), CALL(1)));
        env.close();

        env(pay(alice, alice, CALL(1)), path(gw, bob, ~CALL),
        ...
        env(offer(alice, CALL(1000), bob["USD"](1000)));
        env.close();

        env(pay (carol, carol, gw["USD"](1000)), path(~bob["USD"], gw),
            sendmax(CALL(100000)), txflags(tfNoCallDirect),
            ter(withFix ? tecPATH_DRY : tesSUCCESS));
        env.close();
    ...
    testRIPD1449(bool withFix)
    {
        ...
        // pay alice -> call -> USD/bob -> bob -> gw -> alice
        // set no call on bob's side of the bob/gw trust line
        ...
        env.fund(CALL(100000000), alice, bob, carol, gw);
        ...
        env(offer(carol, CALL(1), bob["USD"](1000)));
        env.close();

        env(pay(alice, alice, USD(1000)), path(~bob["USD"], bob, gw),
            sendmax(CALL(1)), txflags(tfNoCallDirect),
            ter(withFix ? tecPATH_DRY : tesSUCCESS));
        env.close();
    }
    ...
    testSelfPayLowQualityOffer (std::initializer_list<uint256> fs)
    {
        ...
        // CALL than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    void run() override
    {
        ...
        auto testWithFeats = [this](auto&&... fs)
        {
            ...
            testSelfPayment1({fs...});
            testSelfPayment2({fs...});
            testSelfFundedCALLEndpoint(false, {fs...});
            testSelfFundedCALLEndpoint(true, {fs...});
...
BEAST_DEFINE_TESTSUITE(Flow,app,call);
