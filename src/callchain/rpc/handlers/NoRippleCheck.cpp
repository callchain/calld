...
#include <callchain/app/paths/RippleState.h>
...
Json::Value doNoRippleCheck (RPC::Context& context)
{
    ...
    {
        problems.append ("You appear to have set your default callchain flag even though you "
            "are not a gateway. This is not recommended unless you are experimenting");
    }
    else if (roleGateway & ! bDefaultRipple)
    {
        problems.append ("You should immediately set your default callchain flag");
        if (transactions)
        {
    ...
    forEachItemAfter (*ledger, accountID,
        ...
        {
            if (ownedItem->getType() == ltCALLCHAIN_STATE)
            {
                ...
                {
                    problem = "You should clear the no callchain flag on your ";
                    needFix = true;
                }
                else if (! roleGateway & ! bNoRipple)
                {
                    problem = "You should probably set the no callchain flag on your ";
                    needFix = true;
                }
                if (needFix)
                {
