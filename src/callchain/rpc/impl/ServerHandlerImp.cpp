...
#include <callchain/rpc/json_body.h>
#include <callchain/rpc/ServerHandler.h>
#include <callchain/server/Server.h>
#include <callchain/server/impl/JSONRPCUtil.h>
...
#include <callchain/server/SimpleWriter.h>
            ...
            Json::Value const& jv)
{
         ...
         jv[jss::command].asString() != jv[jss::method].asString()))
    {
        ...
        if (jv.isMember(jss::callchainrpc))
            jr[jss::callchainrpc] = jv[jss::callchainrpc];
        ...
        std::string forwardedFor, std::string user)
{
    ...
    if (jsonRPC.isMember(jss::callchainrpc))
        reply[jss::callchainrpc] = jsonRPC[jss::callchainrpc];
    ...
    http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for callchaind</title></head><body><h1>" +
                systemName() + " Test</h1><p>This page shows callchaind http(s) "
