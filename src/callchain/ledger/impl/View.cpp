...
trustCreate (ApplyView& view,
    ...
    uint256 const&  uIndex,             // --> callchain state entry
    SLE::ref        sleAccount,         // --> the account being set.
    const bool      bAuth,              // --> authorize account.
    const bool      bNoRipple,          // --> others cannot callchain through
                 ...
                 beast::Journal j)
{
    ...
    JLOG (j.trace())
        << "trustDelete: Deleting callchain line: low";
    ...
    {
        JLOG (j.trace())
                << "trustDelete: Deleting callchain line: high";
        terResult   = dirDelete (view,
            false,
            uHighNode,
    ...
    JLOG (j.trace()) << "trustDelete: Deleting callchain line: state";
...
TER
callchainCredit (ApplyView& view,
...
static
STAmount
callchainTransferFee (ReadView const& view,
    ...
    beast::Journal j)
{
    if (from != issuer && to != issuer)
    {
        ...
        if (parityRate != rate)
        {
            ...
            JLOG (j.debug()) << "callchainTransferFee:" <<
                " amount=" << amount.getFullText () <<
                " fee=" << fee.getFullText ();
    ...
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    ...
    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        ...
        callchainCredit (view, uSenderID, uReceiverID, saAmount, false, j);
        saActual = saAmount;
        return tesSUCCESS;
    }
    ...
    if (!fix1141 (view.info ().parentCloseTime))
    {
        STAmount const saTransitFee = callchainTransferFee (
    ...
    JLOG (j.debug()) << "callchainSend> " <<
        to_string (uSenderID) <<
        " - > " << to_string (uReceiverID) <<
        " : deliver=" << saAmount.getFullText () <<
        " cost=" << saActual.getFullText ();

    TER terResult   = callchainCredit (view, issuer, uReceiverID, saAmount, true, j);

    if (tesSUCCESS == terResult)
        terResult = callchainCredit (view, uSenderID, issuer, saActual, true, j);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    if (!saAmount.native ())
    {
        ...
        return callchainSend (view, uSenderID, uReceiverID, saAmount, saActual, j);
    }
