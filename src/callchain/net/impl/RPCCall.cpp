...
#include <callchain/json/Object.h>
...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::callchainrpc) && jv[jss::callchainrpc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            ...
            jv1[jss::callchainrpc] = jv[jss::callchainrpc];
        ...
        if (jv.isMember(jss::callchainrpc))
            jv_error[jss::callchainrpc] = jv[jss::callchainrpc];
...
public:
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
            ...
            {   "callchain_path_find",     &RPCParser::parseRipplePathFind,        1,  2   },
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::callchainrpc))
        jv[jss::callchainrpc] = paramsObj[jss::callchainrpc];
    ...
    Config const& config, Logs& logs)
{
    ...
    if (jvRequest.isMember(jss::callchainrpc))
        jvOutput[jss::callchainrpc] = jvRequest[jss::callchainrpc];
