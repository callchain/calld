...
#ifndef CALLCHAIN_PROTOCOL_BOOK_H_INCLUDED
#define CALLCHAIN_PROTOCOL_BOOK_H_INCLUDED
...
template <>
struct hash <callchain::Issue>
    : private boost::base_from_member <std::hash <callchain::Currency>, 0>
    , private boost::base_from_member <std::hash <callchain::AccountID>, 1>
{
private:
    using currency_hash_type = boost::base_from_member <
        std::hash <callchain::Currency>, 0>;
    using issuer_hash_type = boost::base_from_member <
        std::hash <callchain::AccountID>, 1>;

public:
    using value_type = std::size_t;
    using argument_type = callchain::Issue;
...
template <>
struct hash <callchain::Book>
{
private:
    using hasher = std::hash <callchain::Issue>;

    hasher m_hasher;

public:
    using value_type = std::size_t;
    using argument_type = callchain::Book;
...
template <>
struct hash <callchain::Issue>
    : std::hash <callchain::Issue>
{
    using Base = std::hash <callchain::Issue>;
...
template <>
struct hash <callchain::Book>
    : std::hash <callchain::Book>
{
    using Base = std::hash <callchain::Book>;
    // VFALCO NOTE broken in vs2012
    //using Base::Base; // inherit ctors
};
