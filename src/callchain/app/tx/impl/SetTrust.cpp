...
SetTrust::doApply ()
{
    ...
    if (sleRippleState)
    {
        ...
        else
        {
            ...
            JLOG(j_.trace()) << "Modify callchain line";
        }
    }
    // Line does not exist.
    ...
    {
        JLOG(j_.trace()) <<
            "Redundant: Setting non-existent callchain line to defaults.";
    ...
    else
    {
        ...
        JLOG(j_.trace()) <<
            "doTrustSet: Creating callchain line: " <<
            to_string (index);

        // Create a new callchain line.
