...
#include <callchain/app/paths/PathRequest.h>
    ...
    int const level, Json::Value& jvArray)
{
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        JLOG(m_journal.debug()) << iIdentifier
            << " Paths found, calling callchainCalc";
        ...
        if (rc.result () == tesSUCCESS)
        {
            ...
            if (hasCompletion ())
            {
                // Old callchain_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " callchainCalc returns "
                << transHuman(rc.result());
        }
    }
    ...
    std::shared_ptr<RippleLineCache> const& cache, bool fast)
{
    ...
    if (hasCompletion ())
    {
        // Old callchain_path_find API gives destination_currencies
