...
#include <callchain/app/paths/RippleLineCache.h>
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    try
    {
        ...
        auto rc = path::RippleCalc::callchainCalculate (
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        if (! convert_all_)
        {
            ...
            rcInput.partialPaymentAllowed = true;
            rc = path::RippleCalc::callchainCalculate (
                sandbox,
                mSrcAmount,
...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.callchain.com/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
...
bool Pathfinder::isNoRippleOut (STPath const& currentPath)
{
    ...
    // no callchain on the output. A path with no callchain on its output can't be
    // followed by a link with no callchain on its input.
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                auto& callchainLines (mRLCache->getRippleLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (callchainLines.size ());

                for (auto const& item : callchainLines)
                {
