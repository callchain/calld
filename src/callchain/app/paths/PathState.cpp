...
#include <callchain/app/paths/Credit.h>
#include <callchain/app/paths/PathState.h>
...
// - A callchain nodes output issuer must be the node's account or the next node's
//   account.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    // For callchain, non-XRP, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to callchain through issuer's account.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a callchain account. false, iff node is an offer node.
...
/** Check if a sequence of three accounts violates the no callchain constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no callchain on [first]->[second] and
    [second]->[third]
*/
...
{
    // fetch the callchain lines into and out of this node
...
{
    // There must be at least one node for there to be two consecutive callchain
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no callchain constraints could be violated
