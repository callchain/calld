...
#include <callchain/app/paths/cursor/RippleLiquidity.h>
...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {callchainCalc_, pathState_, multi, j_, nodeIndex_};
...
TER PathCursor::advanceNode (bool const bReverse) const
{
    ...
    int loopCount = 0;
    auto viewJ = callchainCalc_.logs_.journal ("View");
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    callchainCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= zero || node().saTakerGets <= zero)
                {
                    ...
                    else if (callchainCalc_.permanentlyUnfundedOffers_.find (index)
                             != callchainCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = callchainCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != callchainCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        callchainCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
