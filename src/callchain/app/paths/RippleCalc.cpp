...
#include <callchain/app/paths/Tuning.h>
#include <callchain/app/paths/cursor/PathCursor.h>
#include <callchain/app/paths/impl/FlowDebugInfo.h>
...
RippleCalc::Output RippleCalc::callchainCalculate (
    PaymentSandbox& view,
    ...
    Input const* const pInputs)
{
    ...
    if (callFlowV1)
    {
        ...
        auto result = rc.callchainCalculate (compareFlowV1V2 ? &flowV1FlowDebugInfo : nullptr);
...
TER RippleCalc::callchainCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    JLOG (j_.trace())
        << "callchainCalc>"
    ...
    {
        JLOG (j_.debug())
            << "callchainCalc: Invalid transaction:"
            << "No paths and direct callchain not allowed.";

        return temCALLCHAIN_EMPTY;
    }
    ...
    JLOG (j_.trace())
        << "callchainCalc: Paths in set: " << spsPaths_.size ();
        ...
        for (auto pathState : pathStateList_)
        {
                ...
                // Only do active paths.
            {
                ...
                else if (pathState->outPass() == zero)
                {
                    ...
                    JLOG (j_.warn())
                        << "callchainCalc: Non-dry path moves no funds";
