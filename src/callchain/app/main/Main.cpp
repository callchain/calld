...
#include <callchain/core/TimeKeeper.h>
...
#include <callchain/beast/core/Time.h>
...
void printHelp (const po::options_description& desc)
{
    std::cerr
        ...
        << "Commands: \n"
           ...
           "     random\n"
           "     callchain ...\n"
           "     callchain_path_find <json> [<ledger>]\n"
           "     version\n"
           "     server_info\n"
    ...
    bool log)
{
    ...
    using namespace callchain::test;
...
int run (int argc, char** argv)
{
    ...
    beast::setCurrentThreadName ("callchaind: main");
    ...
    {
        std::cerr << "callchaind: Incorrect command line syntax." << std::endl;
    ...
    beast::setCurrentThreadName ("callchaind: rpc");
...
int main (int argc, char** argv)
{
...
#ifdef _MSC_VER
    callchain::sha512_deprecatedMSVCWorkaround();
#endif
    ...
    static_assert (gccver >= 50100,
        "GCC version 5.1.0 or later is required to compile callchaind.");
#endif

    static_assert (BOOST_VERSION >= 105700,
        "Boost version 1.57 or later is required to compile callchaind");
    ...
    //
#if CALLCHAIN_DUMP_LEAKS_ON_EXIT
    ...
    std::set_terminate(callchain::terminateHandler);

    auto const result (callchain::run (argc, argv));
