...
more (or less) work for callchaind to process. Those transactions may have
...
behavior of the callchain network. Note that this is not necessarily the
...
automatically as the callchain network's performance improves, allowing
...
not make sense if callchaind is not synced to the network.
...
### [`server_info`](https://callchain.com/build/callchaind-apis/#server-info) command
...
### [`server_state`](https://callchain.com/build/callchaind-apis/#server-state) command
