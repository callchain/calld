...
#ifndef CALL_CORE_CONFIG_H_INCLUDED
#define CALL_CORE_CONFIG_H_INCLUDED

#include <call/basics/BasicConfig.h>
#include <call/basics/base_uint.h>
#include <call/protocol/SystemParameters.h> // VFALCO Breaks levelization
#include <call/beast/net/IPEndpoint.h>
...
public:
    ...
    std::vector<std::string>    IPS;                    // Peer IPs from calld.cfg.
    std::vector<std::string>    IPS_FIXED;              // Fixed Peer IPs from calld.cfg.
    std::vector<std::string>    SNTP_SERVERS;           // SNTP servers from calld.cfg.
    ...
    std::uint64_t                      FEE_DEFAULT = 1000; // 0.001 CALL
