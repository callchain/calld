...
CallCalc::Output CallCalc::callCalculate (
    ...
    // Issuer:
    //      CALL: callAccount()
    //  non-CALL: uSrcAccountID (for any issuer) or another account with
    //           trust node.
    STAmount const& saMaxAmountReq,             // --> -1 = no limit.

    // Issuer:
    //      CALL: callAccount()
    //  non-CALL: uDstAccountID (for any issuer) or another account with
    //           trust node.
    ...
    Input const* const pInputs)
{
    ...
    if (callFlowV1)
    {
        ...
        auto result = rc.callCalculate (compareFlowV1V2 ? &flowV1FlowDebugInfo : nullptr);
...
TER CallCalc::callCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    ...
    {
        JLOG (j_.debug())
            << "callCalc: Invalid transaction:"
            << "No paths and direct call not allowed.";

        return temCALL_EMPTY;
    }

    // Build a default path.  Use saDstAmountReq_ and saMaxAmountReq_ to imply
    // nodes.
    // XXX Might also make a CALL bridge by default.

    JLOG (j_.trace())
        << "callCalc: Paths in set: " << spsPaths_.size ();
        ...
        for (auto pathState : pathStateList_)
        {
                ...
                // Only do active paths.
            {
                ...
                else if (pathState->outPass() == zero)
                {
                    ...
                    JLOG (j_.warn())
                        << "callCalc: Non-dry path moves no funds";
