...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {callCalc_, pathState_, multi, j_, nodeIndex_};
...
TER PathCursor::advanceNode (bool const bReverse) const
{
    ...
    int loopCount = 0;
    auto viewJ = callCalc_.logs_.journal ("View");
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    callCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= zero || node().saTakerGets <= zero)
                {
                    ...
                    else if (callCalc_.permanentlyUnfundedOffers_.find (index)
                             != callCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = callCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != callCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        callCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
