...
#include <call/app/paths/PathState.h>
...
// - A call nodes output issuer must be the node's account or the next node's
//   account.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    {
        // Need to call through issuer's account.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a call account. false, iff node is an offer node.
...
/** Check if a sequence of three accounts violates the no call constrains
    [first] -> [second] -> [third]
...
{
    // fetch the call lines into and out of this node
...
{
    // There must be at least one node for there to be two consecutive call
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no call constraints could be violated
