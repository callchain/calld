    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].call().drops(),
        baseRefLevel,
...
/*
    ...
    0. Is `featureFeeEscalation` enabled?
        ...
        No: Fallback to `call::apply`. Stop.
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    struct MultiTxn
    {
        ...
        CALLAmount fee = beast::zero;
        CALLAmount potentialSpend = beast::zero;
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 CALL.
                    Example 1: If I have 1,000,000 CALL, I can queue
                        a transaction with a 1,000,000 CALL fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 CALL, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 CALL, and I queue
                        10 transactions with 0.1 CALL fee, I have 1 CALL
                        in flight. I can now queue another tx with a
                        999,999 CALL fee. When the first 10 execute,
                        ...
                        999,999 CALL, or get stuck in the queue.
                    Example 3: If I have 1,000,000 CALL, and I queue
                        7 transactions with 3 CALL fee, I have 21 CALL
