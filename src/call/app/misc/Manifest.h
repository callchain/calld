...
#ifndef CALL_APP_MISC_MANIFEST_H_INCLUDED
#define CALL_APP_MISC_MANIFEST_H_INCLUDED
...
/*
    ...
    Suppose the secret keys installed on a Call validator are compromised.  Not
    only do you have to generate and install new key pairs on each validator,
    EVERY calld needs to have its config updated with the new public keys, and
    ...
    signed by the master key.  When a calld peer receives the new manifest,
