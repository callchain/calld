...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::callrpc) && jv[jss::callrpc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            ...
            jv1[jss::callrpc] = jv[jss::callrpc];
        ...
        if (jv.isMember(jss::callrpc))
            jv_error[jss::callrpc] = jv[jss::callrpc];
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::callrpc))
        jv[jss::callrpc] = paramsObj[jss::callrpc];
    ...
    Config const& config, Logs& logs)
{
    ...
    if (jvRequest.isMember(jss::callrpc))
        jvOutput[jss::callrpc] = jvRequest[jss::callrpc];
