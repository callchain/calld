...
#include <call/rpc/json_body.h>
#include <call/rpc/ServerHandler.h>
#include <call/server/Server.h>
#include <call/server/impl/JSONRPCUtil.h>
#include <call/rpc/impl/ServerHandlerImp.h>
...
#include <call/server/SimpleWriter.h>
            ...
            Json::Value const& jv)
{
         ...
         jv[jss::command].asString() != jv[jss::method].asString()))
    {
        ...
        if (jv.isMember(jss::callrpc))
            jr[jss::callrpc] = jv[jss::callrpc];
        ...
        std::string forwardedFor, std::string user)
{
    ...
    if (jsonRPC.isMember(jss::callrpc))
        reply[jss::callrpc] = jsonRPC[jss::callrpc];
    ...
    http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for calld</title></head><body><h1>" +
                systemName() + " Test</h1><p>This page shows calld http(s) "
