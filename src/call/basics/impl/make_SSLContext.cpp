...
// the calld server-server use case, where we only need MITM
...
get_context (std::string const& cipherList)
{
    ...
    // These are the raw DH parameters that Call Labs has
    // chosen for Call, in the binary format needed by
    // d2i_DHparams.
    //
