...
#ifndef CALL_PROTOCOL_STPATHSET_H_INCLUDED
#define CALL_PROTOCOL_STPATHSET_H_INCLUDED
...
public:
    ...
    STPathElement (
        ...
        , mIssuerID (issuer), is_offer_ (isCALL(mAccountID))
    {
        if (!is_offer_)
            mType |= typeAccount;

        if (forceCurrency || !isCALL(currency))
            mType |= typeCurrency;
